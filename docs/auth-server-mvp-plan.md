...
- GitHub token、平文 session ID、平文認証コードは永続化しない。
...
- `users`、`sessions`、`frozen_users` に加え、`clients`、client 単位の `allowed_redirect_uris`、`oauth_states`、ハッシュ化した `auth_codes` を migration に定義する。
- session ID、state、code は Web Crypto で32バイト以上生成する。session ID と認証コードは `SESSION_SECRET` を用いた用途別 HMAC-SHA-256で保存する。
- client secret は平文保存せず、ハッシュ化して保存する。認証時は計算したハッシュを timing-safe な方法で比較する。
...
- MVP では管理 API を実装しない。
- client 登録、secret 生成・ハッシュ化、redirect URI 登録、`frozen_users` 登録は seed script または `wrangler d1 execute` で行い、その手順を README に記載する。
